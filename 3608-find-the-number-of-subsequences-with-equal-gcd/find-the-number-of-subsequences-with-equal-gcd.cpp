...
public:
    ...
    long long mod=1e9+7;
    int dp[202][202][202];
    int find(vector<int>&nums,int i,int gcd1,int gcd2){
        if(i>=n){
            if(gcd1!=-1 && gcd1==gcd2){
                return 1;
            }
        ...
        return dp[i][gcd1+1][gcd2+1]=(find(nums,i+1,newgcd1,gcd2)%mod + find(nums,i+1,gcd1,newgcd2)%mod + find(nums,i+1,gcd1,gcd2)%mod)%mod;
    }
